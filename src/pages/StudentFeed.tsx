...
export function StudentFeed() {
    ...
    const {
        ...
        queryKey: ['studentDocuments', outletContext?.myStudent?.student.studentId || id],
        queryFn: async () => {
            const myStudentId = outletContext?.myStudent?.student.studentId
            const myId = studentId
            if (myStudentId) return getStudentDocuments(myStudentId.toString())
            else if (myId) return getStudentDocuments(myId.toString())
            throw new Error("Can't load document list: no authorized user.")
        },
    })

    const { isLoading: isLoadingStages, error: errorStage, data: stages } = useAllStages()
...
function getDocumentFeedItem(document: IDocumentDTO, selectedStage: number, stages: IStageDTO[]): IFeedElement {
    return {
        date: new Date(document.createdDate),
        iconL: <i className='ri-file-line' />,
        content: <DocumentFeedItem document={document} selectedStage={selectedStage} stages={stages} />,
    }
}
