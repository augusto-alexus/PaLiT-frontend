...
const awsTimeDeltaMs = 0 //  2 * 3600 * 1000
...
export function getHumanReadableDuration(date: Date) {
    ...
    if (deltaMilliseconds < millisecondsInHour) {
        const deltaMinutes = Math.floor(deltaMilliseconds / millisecondsInMinute)
        return `${deltaMinutes} хвилин${
            deltaMinutes % 10 === 1 ?? deltaMinutes !== 11
                ? 'y'
                : deltaMinutes % 10 > 0 && deltaMinutes % 10 < 5
                ? 'и'
                : ''
        } тому`
    }
    if (deltaMilliseconds < millisecondsInDay) {
        ...
        return `${deltaHours} годин${deltaHours === 1 ? 'y' : deltaHours < 5 ? 'и' : ''} тому`
    }
    ...
    if (deltaMilliseconds < millisecondsInYear) {
        ...
        return `${deltaMonths} ${deltaMonths === 1 ? 'місяць' : deltaMonths < 5 ? 'місяці' : 'місяців'} тому`
    }
    const deltaYears = Math.floor(deltaMilliseconds / millisecondsInYear)
    return `${deltaYears} ${deltaYears === 1 ? 'рік' : deltaYears < 5 ? 'роки' : 'років'} тому`
}
